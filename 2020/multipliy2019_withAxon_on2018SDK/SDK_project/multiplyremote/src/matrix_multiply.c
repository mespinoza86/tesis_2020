...
//	Matrix_Multiply(&matrix_array[0], &matrix_array[1], &matrix_result);
	...
	if (rpmsg_send(ept, &matrix_array, sizeof(matrix)) < 0) {
		LPERROR("rpmsg_send failed\n");
	}
	return RPMSG_SUCCESS;
*/

	cellState *temp_cellstate;
	Axon *ready_axon;
	temp_cellstate = (cellState *)malloc(4*sizeof(cellState));
	temp_cellstate = (cellState *) data;

	*ready_axon = CompAxon(temp_cellstate->axon, temp_cellstate->soma.V_soma);

	if (rpmsg_send(ept, ready_axon, 4*sizeof(Axon)) < 0) {
			LPERROR("rpmsg_send failed\n");
	}
