...
//#define MAX_RPMSG_BUFF_SIZE (32 - RPMSG_HEADER_LEN)
...
typedef struct AxonSoma{
	mod_prec axon_V_axon;
	mod_prec axon_Sodium_m_a;
	mod_prec axon_Sodium_h_a;
	mod_prec axon_Potassium_x_a;
	mod_prec soma_g_CaL;
	mod_prec soma_V_soma;
	mod_prec soma_Sodium_m;
	mod_prec soma_Sodium_h;
	mod_prec soma_Calcium_k;
	mod_prec soma_Calcium_l;
	mod_prec soma_Potassium_n;
	mod_prec soma_Potassium_p;
	mod_prec soma_Potassium_x_s;
	mod_prec dend_V_dend;	
...
GapJunctionIP &gjip)
{
	...
	cellState rtos_state;
	cellState * new_state_pointer = new_state;
	//mespinoz std::thread thread_AxonSoma(execute_AxonSomaCalc, N_Size,local_state,new_state_pointer);
	//mespinoz run_GapJunctionIP(N_Size,neighVdend,gjip);
	//mespinoz thread_AxonSoma.join();
	...
	temp_state = (AxonSoma *)malloc(N_Size*sizeof(AxonSoma));
	read_state = (AxonSoma *)malloc(N_Size*sizeof(AxonSoma));
	

	for(int neuron=0;neuron<(N_Size);neuron++){	
		temp_state[neuron].axon_V_axon = local_state[neuron].axon.V_axon;
		temp_state[neuron].axon_Sodium_m_a = local_state[neuron].axon.Sodium_m_a;
		temp_state[neuron].axon_Sodium_h_a = local_state[neuron].axon.Sodium_h_a;
		temp_state[neuron].axon_Potassium_x_a = local_state[neuron].axon.Potassium_x_a;
		temp_state[neuron].soma_g_CaL = local_state[neuron].soma.g_CaL;
		temp_state[neuron].soma_V_soma = local_state[neuron].soma.V_soma;
		temp_state[neuron].soma_Sodium_m = local_state[neuron].soma.Sodium_m;
		temp_state[neuron].soma_Sodium_h = local_state[neuron].soma.Sodium_h;
		temp_state[neuron].soma_Calcium_k = local_state[neuron].soma.Calcium_k;
		temp_state[neuron].soma_Calcium_l = local_state[neuron].soma.Calcium_l;
		temp_state[neuron].soma_Potassium_n = local_state[neuron].soma.Potassium_n;
		temp_state[neuron].soma_Potassium_p = local_state[neuron].soma.Potassium_p;
		temp_state[neuron].soma_Potassium_x_s = local_state[neuron].soma.Potassium_x_s;
		temp_state[neuron].dend_V_dend = local_state[neuron].dend.V_dend;
	}

/*
	for(int neuron=0;neuron<(N_Size);neuron++){
		//new_state[0] = read_state;
		printf("%i temp_state.axon.V_axon es %f\n",neuron,temp_state[neuron].axon_V_axon);
		printf("temp_state.axon.Sodium_m_a es %f\n",temp_state->axon_Sodium_m_a);
		printf("temp_state.axon.Sodium_h_a es %f\n",temp_state->axon_Sodium_h_a);
		printf("temp_state.axon.Potassium_x_a es %f\n",temp_state->axon_Potassium_x_a);
		printf("temp_state.soma.g_CaL es %f\n",temp_state->soma_g_CaL);
		printf("temp_state.soma.V_soma es %f\n",temp_state->soma_V_soma);
		printf("temp_state.soma.Sodium_m es %f\n",temp_state->soma_Sodium_m);
		printf("temp_state.soma.Sodium_h es %f\n",temp_state->soma_Sodium_h);

	}

	printf("After address temp_state is %p\n",temp_state);
*/
	write(fd,temp_state,N_Size*sizeof(AxonSoma));
	//execute_AxonSomaCalc(N_Size,local_state,new_state);
	run_GapJunctionIP(N_Size,neighVdend,gjip);

//	int *ffff;
//	int size_1;
	bytes_rcvd = read(fd, read_state, (N_Size*sizeof(AxonSoma)));
//	printf("Valores obtenidos\n");
//        bytes_rcvd = read(fd, ffff, (sizeof(int)));
//        printf("Valores obtenidos\n");

//	size_1 = *ffff;
//	printf("Int size is %d\n",size_1);
/*
		printf("read_state.axon.Sodium_m_a es %f\n",read_state->axon_Sodium_m_a);
		printf("read_state.axon.Sodium_h_a es %f\n",read_state->axon_Sodium_h_a);
		printf("read_state.axon.Potassium_x_a es %f\n",read_state->axon_Potassium_x_a);
		printf("read_state.soma.g_CaL es %f\n",read_state->soma_g_CaL);
		printf("read_state.soma.V_soma es %f\n",read_state->soma_V_soma);
		printf("read_state.soma.Sodium_m es %f\n",read_state->soma_Sodium_m);
		printf("read_state.soma.Sodium_h es %f\n",read_state->soma_Sodium_h);
*/
...
/*
	...
	for(int neuron=0;neuron<(N_Size);neuron++){
		//new_state[0] = read_state;
		printf("%i read_state.axon.V_axon es %f\n",neuron,read_state[neuron].axon_V_axon);
		printf("read_state.axon.Sodium_m_a es %f\n",read_state[neuron].axon_Sodium_m_a);
		printf("read_state.axon.Sodium_h_a es %f\n",read_state[neuron].axon_Sodium_h_a);
		printf("read_state.axon.Potassium_x_a es %f\n",read_state[neuron].axon_Potassium_x_a);
		printf("read_state.soma.g_CaL es %f\n",read_state[neuron].soma_g_CaL);
		printf("read_state.soma.V_soma es %f\n",read_state[neuron].soma_V_soma);
		printf("read_state.soma.Sodium_m es %f\n",read_state[neuron].soma_Sodium_m);
		printf("read_state.soma.Sodium_h es %f\n",read_state[neuron].soma_Sodium_h);
...
*/
	...
	for(int neuron=0;neuron<(N_Size);neuron++){	
		new_state[neuron].axon.V_axon = read_state[neuron].axon_V_axon;
		new_state[neuron].axon.Sodium_m_a = read_state[neuron].axon_Sodium_m_a;
		new_state[neuron].axon.Sodium_h_a = read_state[neuron].axon_Sodium_h_a;
		new_state[neuron].axon.Potassium_x_a = read_state[neuron].axon_Potassium_x_a;
		new_state[neuron].soma.g_CaL = read_state[neuron].soma_g_CaL;
		new_state[neuron].soma.V_soma = read_state[neuron].soma_V_soma;
		new_state[neuron].soma.Sodium_m = read_state[neuron].soma_Sodium_m;
		new_state[neuron].soma.Sodium_h = read_state[neuron].soma_Sodium_h;
		new_state[neuron].soma.Calcium_k = read_state[neuron].soma_Calcium_k;
		new_state[neuron].soma.Calcium_l = read_state[neuron].soma_Calcium_l;
		new_state[neuron].soma.Potassium_n = read_state[neuron].soma_Potassium_n;
		new_state[neuron].soma.Potassium_p = read_state[neuron].soma_Potassium_p;
		new_state[neuron].soma.Potassium_x_s = read_state[neuron].soma_Potassium_x_s;	        
	}
