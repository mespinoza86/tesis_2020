...
/////////////////////////////////
    ...
    auto elapsed_1 = std::chrono::duration_cast<std::chrono::microseconds>(end_1-begin_1);
    int times_1=elapsed_1.count();

    printf("Time driver elapsed system clock is: %i \n",times_1);
...
////////////////////////////////////////////////////////////
  ...
  for(int step=0;step<sim._simSteps;step++){
    ...
    sim.runStep(gjip);    
    end1 = clock();
    time_spent2[step] = ((double)(end1 - begin1) / CLOCKS_PER_SEC);
    _count++;
  ...
  for (int step=0; step < _count; step++){
	t_average = t_average + time_spent2[step];	
  }
