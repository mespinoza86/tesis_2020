...
#define V_SIZE (1000)//Cantidad de potenciales V
...
//	printf("Initialization completed\n\n");
	...
	*reg_ctrl_ip = 0x01; //start IP
	...
	printf("R_Unrolled_blocks = %i",r_unrolled_blocks);
...
//		printf("Index Row: %d\n",index_matrix_row);
		if(0<n_unrolled_blocks){
			if(index_matrix_row%UNROLL==(UNROLL-1) ){
				//mespinoz printf("Count1 Numero %i\n",count1);
			...
			if(index_matrix_row%UNROLL==(r_unrolled_blocks-1)){
				//mespinoz printf("Count2 Numero %i\n", count2);
				//mespinoz printf("Numero enviado a HW es = %i",4*BLOCK_SIZE*r_unrolled_blocks);
...
//	printf("s2mm addr: 0x%x\n",*reg_addr_s2mm);
	...
	*reg_size_s2mm = n_rows_size;
	//mespinoz printf("mm2s size: %d\n",*reg_size_mm2s);
	//mespinoz printf("s2mm size: %d\n",*reg_size_s2mm);
	//mespinoz printf("mm2s status: 0x%x\n",*reg_status_mm2s);
	//mespinoz printf("s2mm status: 0x%x\n",*reg_status_s2mm);
	//mespinoz printf("ip status: 0x%x\n",*reg_ctrl_ip);
