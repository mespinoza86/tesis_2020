...
static void generate_matrices(int num_matrices,
				unsigned int matrix_size, struct _matrix *p_data)
{
	int	i, j, k;
...
void ui_thread_entry(){
	...
	/*	printf("\r\n **********************************");
		printf("****\r\n");
		printf("\r\n  Mespinoz Matrix multiplication demo Round %d \r\n", i);
		printf("\r\n **********************************");
		printf("****\r\n");
...
//		compute_thread_entry();
	...
	size = sizeof(local_state[neuron].axon);
	struct Axon *temp_axon;
	struct Axon *read_axon;

	temp_axon = (struct Axon *)malloc(4*sizeof(Axon));
	read_axon = (struct Axon *)malloc(4*sizeof(Axon));

	temp_axon->V_axon = local_state[neuron].axon.V_axon;
	temp_axon->Sodium_m_a = local_state[neuron].axon.Sodium_m_a;
	temp_axon->Sodium_h_a = local_state[neuron].axon.Sodium_h_a;
	temp_axon->Potassium_x_a = local_state[neuron].axon.Potassium_x_a;


	//write(fd,temp_axon,4*sizeof(Axon));

	//int bytes_rcvd;
	//bytes_rcvd = read(fd, read_axon, (4*sizeof(Axon)));
	
/*
	printf("####################################################################");
	printf("Neuron: %i\n",neuron);
	printf("local state V_axon = %f\n",local_state[neuron].axon.V_axon);
	printf("V_axon = %f\n",read_axon->V_axon);
	printf("Sodium_m_a = %f\n",read_axon->Sodium_m_a);
	printf("Sodium_h_a = %f\n",read_axon->Sodium_h_a);
	printf("Potassium_x_a = %f\n",read_axon->Potassium_x_a);
	printf("Data received\n");
	printf("####################################################################");
	printf("Neuron: %i\n",neuron);
	printf("local state V_axon = %f\n",local_state[neuron].axon.V_axon);
	printf("V_axon = %f\n",temp_axon->V_axon);
	printf("Sodium_m_a = %f\n",temp_axon->Sodium_m_a);
	printf("Sodium_h_a = %f\n",temp_axon->Sodium_h_a);
	printf("Potassium_x_a = %f\n",temp_axon->Potassium_x_a);
*/
...
GapJunctionIP &gjip)
{
	...
	cellState *temp_state;
	cellState *read_state;

	temp_state = (cellState *)malloc(4*sizeof(cellState));
	read_state = (cellState *)malloc(4*sizeof(cellState));
...
*/
	...
	write(fd,temp_state,4*sizeof(cellState));

	bytes_rcvd = read(fd, read_state, (4*sizeof(cellState)));
